...
MainGame::MainGame(TextureAtlas* texture):m_fileParser(new FileParser()){
    m_texture = texture;
    printf("running maingame constructor");
...
void MainGame::spawnItem(sf::Vector2f position, sf::Vector2i n_offset, sf::String name){
    ...
    for(auto i = items.begin(); i!= items.end(); i++){
        ...
        if(checker){
            offset product = m_fileParser->getProduct(n_item.get_offset(), i->get_offset());
